...
function DebateViewInner({ sessionId }: { sessionId: string }) {
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 overflow-x-hidden font-sans transition-colors duration-300" suppressHydrationWarning>
      {/* Backdrop Hero (dimmed/blurred wide background) */}
      {movieMetadata?.backdrop_path && (
        <div 
          className="fixed inset-0 z-0 bg-cover bg-center pointer-events-none opacity-[0.06] dark:opacity-[0.12] blur-2xl scale-105 transition-opacity duration-1000"
      ...
      <div className="max-w-5xl mx-auto p-4 md:p-8 space-y-8 relative z-10">
            ...
            <div className="w-full md:w-48 shrink-0 rounded-2xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700/50 bg-slate-100 dark:bg-slate-800 aspect-[2/3] relative">
              {movieMetadata.poster_path ? (
                <Image 
                  ...
                  alt={movieMetadata.title || "Poster"}
                  fill
            ...
            <div className="flex-1 space-y-4 pt-2">
              ...
              {movieMetadata.cast && movieMetadata.cast.length > 0 && (
                <div className="space-y-1.5">
                  <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Starring</p>
                  <div className="flex flex-wrap gap-1.5">
                    {movieMetadata.cast.map((actor: string, idx: number) => (
                      <span key={idx} className="text-xs font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/50 px-2.5 py-1 rounded-full">
                        {actor}
                      </span>
                    ))}
                  </div>
              ...
              {movieMetadata.overview && (
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Overview</p>
                  <p className="text-sm text-slate-655 dark:text-slate-300 leading-relaxed max-w-3xl">{movieMetadata.overview}</p>
                </div>
              )}
            </div>
          </motion.div>
        ...
        <div className="space-y-3">
          <h3 className="text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest pl-1">Live Swarm Debate</h3>
                ...
                <div className="space-y-6" aria-live="polite" aria-relevant="additions text">
                  {agentList.length === 0 && (
                    <div className="h-[350px] flex flex-col items-center justify-center text-center space-y-4">
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400">Agents are analyzing the film metadata...</p>
                    </div>
                  )}
                      ...
                      >
                        ...
                        <div className="flex-1 space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-sm text-slate-800 dark:text-white capitalize">{name}</span>
                            {!msg.isComplete && (
                          ...
                          <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-150 dark:border-slate-800/80 p-4 rounded-2xl rounded-tl-none text-slate-700 dark:text-slate-300 text-sm whitespace-pre-wrap leading-relaxed">
                            {msg.content}
                          </div>
                        </div>
                      </motion.div>
                    ...
                    >
                      ...
                      {finalResult.recommendations && finalResult.recommendations.length > 0 && (
                        <div className="pt-4 border-t border-emerald-500/10 space-y-2">
                          <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Suggested Similar Watchlist</p>
                      ...
                      <div className="mt-6 pt-4 border-t border-emerald-500/10">
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                          <h4 className="text-xs font-bold text-slate-555 dark:text-slate-400 uppercase tracking-wider">How was Roger, Aura & Pixel&apos;s debate?</h4>
                          <div className="flex gap-2">
                            <button 
                              ...
                              className="px-3.5 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded-lg text-xs font-bold border border-emerald-500/20 transition-all cursor-pointer"
                            >
                              👍 Good
                            </button>
                            <button 
                              ...
                              className="px-3.5 py-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 rounded-lg text-xs font-bold border border-rose-500/20 transition-all cursor-pointer"
                            >
                              👎 Bad
                            </button>
                          </div>
        ...
        {movieMetadata && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
            ...
            {movieMetadata.trailer_key && (
              <Card className="border border-slate-200 dark:border-slate-800/80 bg-white/95 dark:bg-slate-900/60 backdrop-blur-md rounded-2xl overflow-hidden shadow-lg">
                <CardContent className="p-4 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button 
                      ...
                      className="text-xs font-bold text-blue-655 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300 bg-blue-500/10 border border-blue-500/20 dark:border-blue-500/20 px-3 py-1 rounded-lg transition-all cursor-pointer"
                    >
                  ...
                  {trailerExpanded && (
                    <div className="aspect-video w-full rounded-xl overflow-hidden border border-slate-200 dark:border-slate-850 shadow-2xl bg-black">
                      <iframe
                        ...
                        <div key={idx} className="flex items-center gap-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-150 dark:border-slate-750 p-1.5 pr-3.5 rounded-xl shadow-sm hover:border-slate-300 dark:hover:border-slate-600 transition-colors">
                          {provider.logo_path ? (
                            <Image 
                              ...
                              alt={provider.name || "Watch provider logo"}
                              width={28}
                              height={28}
                              className="rounded-md object-contain border border-slate-200 dark:border-slate-700"
                            />
                          ) : (
        ...
        {movieMetadata?.similar_movies && movieMetadata.similar_movies.length > 0 && (
          <div className="space-y-4 pt-4 animate-fade-in">
            <h3 className="text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest pl-1">You Might Also Like</h3>
                  ...
                  >
                    <div className="aspect-[2/3] w-full rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800 relative shadow-md">
                      {movie.poster_path ? (
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-600 p-2 text-center bg-slate-50 dark:bg-slate-900">
                          <span className="text-2xl">🎬</span>
                          <span className="text-[10px] font-bold mt-1 leading-tight">No Poster</span>
                        </div>
                      )}
                    </div>
                    <p className="text-xs font-black text-slate-700 dark:text-slate-300 truncate text-center px-1 group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors">
                      {movie.title}
                    </p>
                  </div>
                ))}
